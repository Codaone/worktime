...
$rows 	= $db->get("SELECT * FROM `".$table."` ".$cond." ORDER BY `begin` DESC, starttime DESC".$page);
$count 	= $db->rows($table);
$total 	= 0;
$days	= array();
?>
...
$weeklyprice = 0;
$usersArr = array();
$tmpusersArr = $db->get("SELECT id,name FROM users");
foreach($tmpusersArr as $row) {
	$usersArr[$row["id"]] = $row["name"];
}
...
if($user->getUsername() == "admin") {
	$data = $db->get("SELECT
		...
		FROM `work` WHERE FROM_UNIXTIME(`begin`, '%Y') = '".date("Y")."'");
} else {
	$data = $db->get("SELECT
		SUM(duration)/3600 'Total (h)',
		SUM(IF(charged, 0, duration))/3600 'Uncharged (h)',
...
if(getvar('del') && getvar('del')!="succeed"){
	...
	$id = getvar('id');
	if(strlen($id) && $id > 0){
		$delete = "DELETE FROM `".$table."` WHERE `id` = ".$db->escape($id)." LIMIT 1";
		$db->query($delete);
